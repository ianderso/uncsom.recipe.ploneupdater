...
class Recipe(object):
    ...
    def install(self):
        ...
        migrate_plone = 'migrate-plone' in self.options and \
            self.options['migrate-plone'].lower() in \
            ['true', 'on', 'yes'] or False
        ...
        uninstall = 'uninstall' in self.options and \
            [p for p in self.options['uninstall'].splitlines() if p] or []
        ...
        profiles = 'run-profile' in self.options and \
            [p for p in self.options['run-profile'].splitlines() if p] or []
